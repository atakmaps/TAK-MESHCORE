...
public class MeshCoreMapComponent extends DropDownMapComponent {
    ...
    private void maybeStartBeaconTimer() {
        if (SettingsFragment.isMeshBeaconEnabled(getBeaconPrefsContext())) {
            startBeaconTimer();
        } else {
            stopBeaconTimer();
        }
    ...
    private void startBeaconTimer() {
        ...
        if (hasValidSelfPosition()) {
            Log.d(TAG, "Mesh beacon timer armed: valid self position already present");
        ...
        beaconWaitForPositionRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (hasValidSelfPosition()) {
                    Log.d(TAG, "Valid self position acquired; mesh startup beacon in 30s");
                    beaconHandler.postDelayed(beaconRunnable, 30_000L);
        ...
        Log.d(TAG, "Mesh beacon timer waiting for valid self position");
    ...
    private void sendBeaconIfConnected(boolean forceImmediate) {
        ...
        try {
            ...
            if (!forceImmediate && SmartBeacon.isEnabled(beaconCtx)) {
                ...
                Log.d(TAG, "Mesh smart beacon check: speed="
                        + String.format(Locale.US, "%.1f", speedMph)
                        + "mph (src=" + speedSrc + ")"
                ...
                Log.d(TAG, "Mesh post-connect startup beacon fired (30s)");
            }
            ...
            Log.e(TAG, "Error sending mesh periodic beacon", e);
        }
    }
    ...
    private boolean hasValidSelfPosition() {
        ...
        return gp != null && gp.isValid()
                && isValidCoordinate(gp.getLatitude(), gp.getLongitude());
    }
